...
def prop_to_pixel(points, width, height):
    points_prop = []
    for point in points:
        points_prop.append([point[0]*height, point[1]*width*0.90]) # fixed
...
def seat_occupation(space, points, labels, occupation):
    occupation_final = occupation.copy()
    for i in range(len(space)):
        flag = 0
        for p in range(len(points)):
            if (space[i][0] <= points[p][0]  <= space[i][2] 
                and space[i][1] <= points[p][1] <= space[i][3]):
                if labels[p] == 0: # 사람이면
